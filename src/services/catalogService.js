...
async function loadRowsWithProgressiveSelects(api, candidates, params = {}, pageSize = 160, selectVariants = CATALOG_SELECT_VARIANTS, options = {}) {
  ...
  for (const variant of createProgressiveSelectOptions(params, selectVariants)) {
    try {
      const rows = await loadRowsWithFallback(api, candidates, variant, pageSize, cacheKey, options);
...
async function loadPagedRowsVariant(api, path, params = {}, pageSize = 160, options = {}) {
  const rows = [];
  let offset = 0;
  const requestParams = { ...params };
  while (true) {
    if (rows.length >= 300) break;
    const page = await api.get(path, { ...requestParams, limit: String(pageSize), offset: String(offset) }, options);
...
async function loadPagedRows(api, path, params = {}, pageSize = 160, options = {}) {
  let lastError = null;
  for (const strategy of buildLoadStrategies(params)) {
    try {
      return await loadPagedRowsVariant(api, path, strategy, pageSize, options);
    } catch (error) {
      lastError = error;
...
async function loadRowsWithFallback(api, candidates, params = {}, pageSize = 160, cacheKey = '', options = {}) {
  let lastError = null;
  ...
  for (const path of orderedCandidates) {
    try {
      const rows = await loadPagedRows(api, path, params, pageSize, options);
...
export async function loadHomeCatalog(api, selectedTierName = null, options = {}) {
  ...
  const companiesRequest = api.get('companies', { select: 'company_id,company_name,company_logo,visible,allow_discount', order: 'company_id.asc' }, { signal });
  const tiersRequest = api.get('tiers', { select: 'tier_name,visible_label,min_order,discount_percent,visible,is_active,is_default,discount_carton,discount_pack', order: 'min_order.asc' }, { signal });
  const settingsRequest = (!criticalOnly && includeProducts) ? api.get('v_app_settings', { select: 'settings,updated_at' }, { signal }) : Promise.resolve([]);
  const dailyRequest = (!criticalOnly && includeProducts) ? api.get('v_daily_deals', { select: 'id,title,description,image,price,stock,can_buy,is_active,sold_count' }, { signal }) : Promise.resolve([]);
  const flashRequest = (!criticalOnly && includeProducts) ? api.get('v_flash_offers_runtime', { select: '*' }, { signal }) : Promise.resolve([]);
  const catalogRowsRequest = includeProducts ? loadRowsWithProgressiveSelects(api, ['v_catalog_products', 'v_runtime_products_safe', 'v_runtime_products_mobile'], {
  ...
  const topProductsRequest = includeTop && !criticalOnly && includeProducts
    ? api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc' }, { signal }).catch(() => [])
    : Promise.resolve([]);
  const topCompaniesRequest = includeTop && !criticalOnly && includeProducts
    ? api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc' }, { signal }).catch(() => [])
    : Promise.resolve([]);
  ...
  const result = {
    ...
    counters: {
      ...
      deals: (!criticalOnly && includeProducts) ? daily.length : 0,
      flash: (!criticalOnly && includeProducts) ? flash.length : 0,
    },
  };
...
export async function loadCompanyCatalog(api, companyId, selectedTierName = null, options = {}) {
