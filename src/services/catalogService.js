...
async function loadPagedRows(api, path, params = {}, pageSize = 160, maxPages = 100) {
  const rows = [];
  let offset = 0;
  let pagesFetched = 0;
  while (pagesFetched < maxPages) {
    ...
    if (batch.length < pageSize || nextOffset === offset) break;
    offset = nextOffset;
    await sleepFrame();
  }
  return rows;
...
export async function loadHomeCatalog(api, selectedTierName = null) {
  ...
  const catalogRows = await loadPagedRows(api, 'v_runtime_products_mobile', {
  ...
  const topProducts = await api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc' }).catch(() => []);
  const topCompanies = await api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc' }).catch(() => []);
  ...
  const catalogIndex = projectCatalogProducts(aggregateCatalogProducts(catalogRows), selectedTierName);
