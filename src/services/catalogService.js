...
async function loadRowsWithProgressiveSelects(api, candidates, params = {}, pageSize = 160, selectVariants = CATALOG_SELECT_VARIANTS, maxRows = 300) {
  ...
  for (const variant of createProgressiveSelectOptions(params, selectVariants)) {
    try {
      const rows = await loadRowsWithFallback(api, candidates, variant, pageSize, cacheKey, maxRows);
...
async function loadPagedRowsVariant(api, path, params = {}, pageSize = 160, maxRows = 300) {
  const rows = [];
  let offset = 0;
...
async function loadPagedRows(api, path, params = {}, pageSize = 160, maxRows = 300) {
  let lastError = null;
  for (const strategy of buildLoadStrategies(params)) {
    try {
      return await loadPagedRowsVariant(api, path, strategy, pageSize, maxRows);
    } catch (error) {
      lastError = error;
...
async function loadRowsWithFallback(api, candidates, params = {}, pageSize = 160, cacheKey = '', maxRows = 300) {
  let lastError = null;
  ...
  for (const path of orderedCandidates) {
    try {
      const rows = await loadPagedRows(api, path, params, pageSize, maxRows);
...
export async function loadHomeCatalog(api, selectedTierName = null, options = {}) {
  ...
  const catalogRowsRequest = criticalOnly
    ? Promise.resolve([])
    : loadRowsWithProgressiveSelects(api, ['v_catalog_products', 'v_runtime_products_safe', 'v_runtime_products_mobile'], {
  ...
  const topProductsRequest = criticalOnly ? Promise.resolve([]) : (includeTop ? api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc' }).catch(() => []) : Promise.resolve([]));
  const topCompaniesRequest = criticalOnly ? Promise.resolve([]) : (includeTop ? api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc' }).catch(() => []) : Promise.resolve([]));
...
export async function loadCompanyCatalog(api, companyId, selectedTierName = null, options = {}) {
  ...
  const maxRows = Number(options?.maxRows || pageSize);
