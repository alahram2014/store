...
export function buildPriceBook(products, tiers, selectedTierName) {
  const normalizedTier = normalizeTierName(selectedTierName) || normalizeTierName(tiers.find((tier) => tier.is_default)?.tier_name) || 'base';
  const book = {};
...
export function computeDisplayPrice(product, unit) {
  ...
  const base = Number.isFinite(baseCandidate) && baseCandidate > 0 ? Number(baseCandidate.toFixed(2)) : final;
  return { base, final };
}
...
export function syncCartPrices(cart, productsById) {
  return (Array.isArray(cart) ? cart : []).map((item) => {
    ...
    return {
      ...
      tierName: normalizeTierName(record?.tier_name || item.tierName || null),
    };
  });
}
...
export function persistSelectedTier(tierName) {
  localStorage.setItem(storageKeys.tier, JSON.stringify(normalizeTierName(tierName) || null));
}
