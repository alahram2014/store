...
export function getOwnershipActorId(session) {
  ...
  if (isSalesRepSession(normalized)) return candidate;
  if (hasOperationalAccess(normalized)) return normalizeIdentifier(normalized?.system_user?.id || normalized?.id || candidate);
  if (normalized?.sales_rep_id || normalized?.rep_id || normalized?.created_by_rep_id) return candidate;
  return null;
}
...
async function queryCapabilityProjectionRows(api, normalizedSession, identifier) {
  const resources = [
    ...
    'system_users_capabilities',
    'system_user_capabilities',
  ];
  const filters = [];
  ...
  const systemUserId = normalizeIdentifier(
    ...
    || normalizedSession?.user_id
    || normalizedSession?.auth_user_id
    || '',
  );
  if (systemUserId) {
    filters.push({ system_user_id: `eq.${systemUserId}` });
    filters.push({ user_id: `eq.${systemUserId}` });
    filters.push({ id: `eq.${systemUserId}` });
  }
...
async function enrichOperationalSession(api, session) {
  ...
  const identifier = normalizeIdentifier(normalizedSession?.username || normalizedSession?.phone || normalizedSession?.system_user?.username || '');
