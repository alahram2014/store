...
export function normalizeSessionRecord(session) {
  ...
  const salesRepId = normalizeIdentifier(
...
async function fetchUserProfile(api, table, identifier) {
  ...
  const select =
    PROFILE_SELECT[table]
    || PROFILE_SELECT.customers;
  ...
  const rows = await api.get(table, {
    ...
    const phone = await api.get(table, {
      select,
      phone: `eq.${trimmed}`,
      limit: '1',
    }).catch(() => []);
...
async function fetchIdentityProfiles(api, identifier) {
  ...
  const results = await Promise.allSettled(
    tables.map((table) =>
      fetchUserProfile(api, table, identifier),
    ),
  );
...
function resolveAuthoritativeUserType(authenticated) {
  const authType = normalizeUserType(
    authenticated?.userType
      || authenticated?.user_type
      || authenticated?.role
      || null,
    null,
  );
...
export async function login(api, identifier, password) {
  ...
  const authenticated = normalizeSessionRecord(
  ...
  const profileMap = Object.fromEntries(
    profiles.map((entry) => [entry.table, entry.row]),
  );

  const authoritativeType =
    resolveAuthoritativeUserType(authenticated);
  ...
  const authoritativeTable =
    USER_TYPE_TO_TABLE[authoritativeType];

  const authoritativeProfile =
    authoritativeTable
      ? profileMap[authoritativeTable] || null
      : null;

  const fallbackProfile =
    authoritativeProfile || authenticated;
  ...
  if (
    normalizeUserType(session?.userType, null)
    !== authoritativeType
  ) {
    throw new Error('AUTH_ROLE_UNRESOLVED');
  }
...
export function currentSession() {
  return normalizeSessionRecord(
    loadJSON(storageKeys.session, null),
  );
}

export async function registerCustomer(api, payload) {
  const exists = await api.get('customers', {
