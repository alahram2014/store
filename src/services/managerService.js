...
export async function loadManagerScopeIntoState(store, api, session = null, { force = false } = {}) {
  ...
  store.update((draft) => {
    ...
    if (scope.loaded && hasOperationalAccess(session || current.auth.session || {})) {
      draft.commerce.customers = Array.isArray(scope.teamCustomers) ? scope.teamCustomers.map((entry) => ({ ...entry })) : [];
    }
