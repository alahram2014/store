...
export function getDefaultOperationalModule() {
...
export function getOperationalQuickActions(session = {}) {
  return [
    ...
    },
    {
      ...
      visible: isSalesRepSession(session),
    },
    {
      key: 'checkout',
      label: 'إنشاء طلب',
  ...
  ].filter((action) => action.visible).map((action) => ({ ...action }));
}
...
export async function loadManagerScopeIntoState() {
  return {
    loaded: false,
    loading: false,
    error: null,
