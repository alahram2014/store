...
import { getOwnershipActorId, hasOperationalAccess, isSalesRepSession, normalizeUserType } from './authService.js';
...
export function validateCheckout(state, tier, totals) {
  ...
  if (userType !== 'customer' && hasOperationalAccess(session) && !state.auth.selectedCustomer) return { ok: false, code: 'NO_CUSTOMER', message: 'اختر العميل أولاً' };
...
export async function submitOrder(api, state, tier, totals) {
  ...
  const orderPayload = {
    user_type: isCustomerSession ? 'customer' : userType,
    ...
    rep_id: salesRepId,
    customer_type: customer?.customer_type || (isCustomerSession ? 'direct' : 'managed'),
