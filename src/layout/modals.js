...
export function renderInvoiceModal(state) {
  ...
  const mapsUrl = invoice.customer_google_maps_url || (Number.isFinite(Number(invoice.customer_location_lat)) && Number.isFinite(Number(invoice.customer_location_lng))
    ? `https://maps.google.com/?q=${Number(invoice.customer_location_lat)},${Number(invoice.customer_location_lng)}`
    : '');
  return `
    <section class="modal-overlay modal-overlay--center is-open" data-modal="invoice" role="dialog" aria-modal="true" tabindex="-1">
      ${modalFrame(`فاتورة #${invoice.order_number || invoice.invoice_number || invoice.id}`, `
        <div class="invoice-preview">
          ...
          <div class="invoice-preview__row"><span>النوع</span><strong>${invoice.customer_type === 'rep' ? 'عميل مندوب' : 'عميل مباشر'}</strong></div>
          <div class="invoice-preview__row"><span>المندوب</span><strong>${invoice.sales_rep_name || invoice.rep_name || '—'}</strong></div>
          <div class="invoice-preview__row"><span>رقم المندوب</span><strong>${invoice.sales_rep_phone || invoice.rep_phone || '—'}</strong></div>
          <div class="invoice-preview__row"><span>العنوان</span><strong>${invoice.customer_location || 'غير محدد'}</strong>${mapsUrl ? `<a class="btn btn--ghost" href="${mapsUrl}" target="_blank" rel="noopener noreferrer">Google Maps</a>` : ''}</div>
