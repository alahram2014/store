import { toastStack } from '../components/feedback.js';
import { formatMoney } from '../services/invoiceService.js';

export function renderDrawer(state) {
  const items = state.commerce.cart || [];
  return `
    ${state.ui.drawerOpen ? '<button class="drawer-backdrop" type="button" data-action="close-cart-drawer" aria-hidden="true"></button>' : ''}
    <aside class="drawer ${state.ui.drawerOpen ? 'is-open' : 'is-hidden'}" id="drawerPanel" role="dialog" aria-modal="true" aria-label="السلة">
      <div class="drawer__head">
        <h3>السلة</h3>
        <button class="icon-btn" type="button" data-action="close-cart-drawer">×</button>
      </div>
      <div class="drawer__body">
        ${items.length ? items.map((item) => `
          <article class="drawer-item">
            <div>
              <h4>${item.title || ''}</h4>
              <p>${item.unitLabel || item.unit || ''} · ${Number(item.qty || 1)} × ${Number(item.price || 0)}</p>
            </div>
            <button class="btn btn--ghost" type="button" data-action="remove-item" data-key="${item.key}">حذف</button>
          </article>
        `).join('') : '<div class="empty-state">السلة فارغة</div>'}
      </div>
      <div class="drawer__foot">
        <button class="btn btn--primary" type="button" data-action="go-checkout">إتمام الشراء</button>
        <button class="btn btn--ghost" type="button" data-action="go-cart">إغلاق السلة ومتابعة الشراء</button>
      </div>
    </aside>
  `;
}

export function renderFloatingCart(state) {
  const items = state.commerce.cart || [];
  const total = items.reduce((sum, item) => sum + Number(item.price || 0) * Number(item.qty || 0), 0);
  if (!items.length && Number(total || 0) <= 0) return '';
  return `
    <button class="floating-cart" type="button" data-action="go-checkout" aria-label="مراجعة الطلب">
      <span class="floating-cart__label">إجمالي السلة</span>
      <strong class="floating-cart__value">${formatMoney(total)} ج.م</strong>
    </button>
  `;
}

export function renderToasts(state) {
  return `
    <div class="toast-stack">
      ${toastStack(state.ui.toastQueue || [])}
    </div>
  `;
}
