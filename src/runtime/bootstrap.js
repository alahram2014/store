...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'go-checkout') {
      const sessionType = normalizeUserType(state.auth.session?.userType || state.auth.session?.user_type || state.auth.session?.role || null, null);
      const requiresCustomerSelection = sessionType !== 'customer' && hasOperationalAccess(state.auth.session);
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'customer-create') {
      ...
      if (!hasOperationalAccess(session) && !isManagerScope) return notify(store, 'warning', 'الحساب التشغيلي فقط', '');
      ...
      const payload = {
        ...
        customer_type: isManagerScope || hasOperationalAccess(session) ? 'managed' : 'direct',
        ...
        owner_user_id: isManagerScope ? (session?.system_user?.id || session.id) : session.id,
        owner_user_type: isManagerScope ? 'sales_manager' : (isSalesRepSession(session) ? 'sales_rep' : 'admin'),
        owner_scope: isManagerScope ? 'sales_manager' : (isSalesRepSession(session) ? 'sales_rep' : 'admin'),
        is_active: true,
      };
  ...
  if (hasOperationalAccess(session)) {
    ...
    if (!rows.length) {
      ...
      rows = await api.get('orders', {
        ...
        or: `(sales_rep_id.eq.${ownershipActorId},rep_id.eq.${ownershipActorId})`,
...
export async function bootstrapApp() {
  ...
  nodes.floating.addEventListener('focusin', () => {
    if (shouldRenderFloatingExecutionBar(store.getState())) {
      const currentRoute = store.getState().app.route.name;
      if (currentRoute !== 'search') {
        navigateAuthority(store, 'search');
      }
    }
  });
  nodes.floating.addEventListener('input', (event) => {
    const input = event.target.closest('[data-role="floating-search-input"]');
    if (!input) return;
    const nextValue = String(input.value || '');
    store.patch({ ui: { ...store.getState().ui, search: nextValue } });
    if (store.getState().app.route.name !== 'search') {
      navigateAuthority(store, 'search');
    } else {
      schedule('search', 'page', 'header', 'hero', 'floating');
    }
  });
  ...
  void (async () => {
    ...
    if (currentSession) {
      try {
        ...
        if (projectedSession) {
          ...
          const capabilitiesChanged = JSON.stringify(activeSession?.capabilities || []) !== JSON.stringify(nextSession?.capabilities || []);
          const domainsChanged = JSON.stringify(activeSession?.domains || []) !== JSON.stringify(nextSession?.domains || []);
          const userChanged = activeSession?.id !== nextSession?.id || activeSession?.userType !== nextSession?.userType;
          if (capabilitiesChanged || domainsChanged || userChanged) {
            store.patch({
              auth: {
                ...activeState.auth,
                session: nextSession,
    ...
    if (hasOperationalAccess(session)) {
      ...
      if (hasOperationalAccess(session) || isSalesRepSession(session)) {
        store.update((draft) => { draft.runtime.loading.customers = true; }, { silent: true });
        void loadCustomersIntoState(store, api, session);
      }
      store.update((draft) => { draft.runtime.loading.invoices = true; }, { silent: true });
