...
import { login, logout, registerCustomer, normalizeUserType, normalizeSessionRecord, isSalesRepSession, getOwnershipActorId, hasCapability } from '../services/authService.js';
import { loadRepCustomers, createCustomer, persistSelectedCustomer } from '../services/customerService.js';
import { loadManagerScopeIntoState, hasOperationalAccess, getDefaultOperationalModule } from '../services/managerService.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'go-ops') return navigateAuthority(store, 'ops', { module: getDefaultOperationalModule(state.auth.session) });
