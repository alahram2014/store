...
import { login, logout, registerCustomer, normalizeUserType, normalizeSessionRecord, isSalesRepSession, getOwnershipActorId, persistSessionRecord, readPersistedSession, refreshSessionProjection } from '../services/authService.js';
...
function renderPage(state, nodes) {
  ...
  switch (route.name) {
    ...
    case 'ops':
    case 'sales-manager': pageHtml = renderHomePage(state); break;
...
function bindInteractions(store, api, schedule) {
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'customer-create') {
      ...
      if (!isSalesRepSession(session)) return notify(store, 'warning', 'الحساب التشغيلي فقط', '');
      const rawLat = String(form.location_lat?.value || '').trim();
      const rawLng = String(form.location_lng?.value || '').trim();
      const ownershipActorId = getOwnershipActorId(session) || session?.sales_rep_id || session?.rep_id || session?.id || '';
      const payload = {
        name: String(form.name?.value || '').trim(),
        ...
        owner_user_id: ownershipActorId,
        owner_user_type: 'sales_rep',
        owner_scope: 'sales_rep',
        is_active: true,
      };
          ...
          if (capabilitiesChanged || domainsChanged || userChanged) {
            store.patch({
              auth: {
                ...activeState.auth,
                session: nextSession,
                selectedCustomer: isSalesRepSession(nextSession) ? activeState.auth.selectedCustomer : null,
              },
            });
