...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      // runtime-only company rows cache; no localStorage persistence
      persistCart(store.getState().commerce.cart);
      return;
    }
    ...
    if (companyHydrationTokens.get(trimmed) !== requestToken) return;
    // runtime-only company rows cache; no localStorage fallback
    store.update((draft) => {
      draft.runtime.loading.company = null;
...
export async function bootstrapApp() {
  ...
  try {
    ...
    const catalog = liveCatalog && typeof liveCatalog === 'object' ? liveCatalog : createEmptyCatalog();
    ...
    setRuntimePhase(store, RUNTIME_PHASES.SYNCING_CART, {
      ...
      companyProductsLoading: Boolean(store.getState().runtime.lifecycle?.companyProductsLoading),
    ...
    console.error(error);
    const selectedTier = normalizeTierName(store.getState().commerce.selectedTier) || 'base';
    const cart = hydrateCart();
    store.patch({
      commerce: {
        ...store.getState().commerce,
      ...
      runtime: { ...store.getState().runtime, loading: { ...store.getState().runtime.loading, catalog: false, pricing: true }, flashState: computeFlashState([]) },
      app: { ...store.getState().app, lastError: null },
    });
    setRuntimePhase(store, RUNTIME_PHASES.SYNCING_CART, {
      catalogReady: true,
  ...
  setRuntimePhase(store, RUNTIME_PHASES.READY, { locked: false, catalogReady: true, offersReady: true, flashOffersReady: true, companiesReady: true, pricingReady: true, cartSynced: true, companyProductsReady: Boolean(store.getState().runtime.lifecycle?.companyProductsReady), companyProductsLoading: false, companyProductsFailed: Boolean(store.getState().runtime.lifecycle?.companyProductsFailed) });
  store.update((draft) => {
