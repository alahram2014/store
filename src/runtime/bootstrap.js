  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'customer-create') {
      ...
      const payload = {
        ...
        owner_user_id: isManagerScope ? (session?.system_user?.id || session.id) : (session?.system_user?.id || session.id),
        owner_user_type: isManagerScope ? 'sales_manager' : (normalizeUserType(session?.userType || session?.user_type || session?.role || null, null) || 'admin'),
        owner_scope: hasOperationalAccess(session) ? 'operational' : (isManagerScope ? 'sales_manager' : 'admin'),
        is_active: true,
      };
          ...
          if (capabilitiesChanged || domainsChanged || userChanged) {
            store.patch({
              auth: {
                ...activeState.auth,
                session: nextSession,
                selectedCustomer: hasOperationalAccess(nextSession) ? activeState.auth.selectedCustomer : null,
              },
            });
