...
import { login, logout, registerCustomer, normalizeUserType, normalizeSessionRecord, isSalesRepSession, getOwnershipActorId } from '../services/authService.js';
import { loadRepCustomers, createCustomer, persistSelectedCustomer } from '../services/customerService.js';
import { buildGoogleMapsUrl, projectInvoiceRuntimeRecord, projectCustomerOwnership, isRepManagedCustomer, resolveSalesRepContext } from '../services/ownershipService.js';
...
function getNodes() {
  return {
    ...
    floatingCartHost: dom.q('#appFloatingCartHost'),
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'go-checkout') {
      const canUseOperationalCustomerFlow = isSalesRepSession(state.auth.session) || Boolean(state.auth.session?.system_user);
      if (canUseOperationalCustomerFlow && !state.auth.selectedCustomer) {
    ...
    if (action === 'view-invoice') {
      ...
      const invoiceCustomer = invoice.customer_id
        ? projectCustomerOwnership((store.getState().commerce.customers || []).find((customer) => String(customer.id) === String(invoice.customer_id)) || {
            id: invoice.customer_id,
            name: invoice.customer_name || '',
            phone: invoice.customer_phone || '',
            location: invoice.customer_location || '',
            location_lat: invoice.customer_location_lat ?? null,
            location_lng: invoice.customer_location_lng ?? null,
            customer_type: invoice.customer_type || 'direct',
          })
        : null;
      store.update((draft) => {
        ...
        if (invoiceCustomer) {
          draft.commerce.invoices = (draft.commerce.invoices || []).map((row) => String(row.id) === String(invoiceId)
            ? projectInvoiceRuntimeRecord(row, { customer: invoiceCustomer, session: draft.auth.session })
            : row);
        }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'login') {
      ...
      try {
        ...
        Promise.resolve().then(() => {
          ...
          schedule('header', 'banner', 'page', 'drawer', 'floatingCart', 'search', 'hero', 'modals');
        });
      } catch (error) {
        const persistedSession = normalizeSessionRecord(loadJSON(storageKeys.session, null));
    ...
    if (formType === 'customer-create') {
      ...
      const ownershipContext = await resolveSalesRepContext(api, session).catch(() => ({ salesRepId: '', salesRepProfile: null }));
      const ownershipActorId = ownershipContext.salesRepId || getOwnershipActorId(session) || session.id;
      const payload = {
        name: String(form.name?.value || '').trim(),
        ...
        customer_type: 'rep',
        sales_rep_id: ownershipActorId,
        created_by: session.id,
        created_by_rep_id: ownershipActorId,
        is_active: true,
      };
      ...
      try {
        ...
        if (pendingFlow?.name === 'checkout') {
          ...
          schedule('page', 'header', 'modals', 'floatingCart');
        }
      } catch {
...
async function loadInvoicesIntoState(store, api) {
  ...
  const repScope = await resolveSalesRepContext(api, session).catch(() => ({ salesRepId: '', salesRepProfile: null }));

  if (repScope.salesRepId) {
    const ownershipActorId = repScope.salesRepId;
    ...
    customerMap = new Map(normalizedCustomers.map((customer) => [String(customer.id), projectCustomerOwnership(customer)]));
    const customerIds = Array.from(new Set(normalizedCustomers.map((customer) => String(customer.id || '').trim()).filter(Boolean)));
    const filters = [
      `sales_rep_id.eq.${ownershipActorId}`,
      `rep_id.eq.${ownershipActorId}`,
      `created_by_rep_id.eq.${ownershipActorId}`,
      `created_by.eq.${ownershipActorId}`,
    ];
    if (customerIds.length) filters.push(`customer_id.in.(${customerIds.join(',')})`);
    rows = await api.get('orders', {
      select: 'id,order_number,invoice_number,created_at,total_amount,status,user_type,customer_type,customer_id,user_id,sales_rep_id,rep_id,created_by_rep_id,created_by,customer_name,customer_phone,customer_location,customer_location_lat,customer_location_lng,sales_rep_name,sales_rep_phone,updated_at',
      or: `(${filters.join(',')})`,
      order: 'created_at.desc',
    }).catch(() => []);

    rows = Array.isArray(rows)
      ? rows.map((row) => projectInvoiceRuntimeRecord(row, {
    ...
    rows = Array.isArray(rows)
      ? rows.map((row) => projectInvoiceRuntimeRecord(row, {
          customer: projectCustomerOwnership({
            id: session.id,
            ...
            customer_type: session.customer_type || 'direct',
          }),
          session,
        }))
      : [];
...
async function loadCustomersIntoState(store, api, session = null) {
  const state = store.getState();
  const rep = normalizeSessionRecord(session || state.auth.session);
  const scope = await resolveSalesRepContext(api, rep).catch(() => ({ salesRepId: '', salesRepProfile: null }));
  if (!scope.salesRepId) {
    store.update((draft) => { draft.commerce.customers = []; draft.runtime.loading.customers = false; });
    return;
  }
  const rows = await loadRepCustomers(api, scope.salesRepId).catch(() => []);
...
async function performCheckout(store, api, schedule) {
  ...
  try {
    ...
    const whatsappUrl = await buildWhatsAppInvoice({
      api,
      order: result.order,
...
export async function bootstrapApp() {
  ...
  function renderContent() {
    ...
    if (booting) {
      ...
      nodes.floatingCartHost.innerHTML = '';
    ...
    nodes.drawerHost.innerHTML = renderDrawer(state);
    nodes.floatingCartHost.innerHTML = renderFloatingCart(state);
    nodes.toastHost.innerHTML = renderToasts(state);
  ...
  store.subscribe((_, meta = {}) => {
    const dirty = Array.isArray(meta.dirty) && meta.dirty.length ? meta.dirty : ['header', 'theme', 'search', 'hero', 'footer', 'page', 'drawer', 'floatingCart', 'modals', 'toast'];
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (nextRoute.name === 'customers') {
      void loadCustomersIntoState(store, api, normalizeSessionRecord(store.getState().auth.session));
    }
    if (nextRoute.name === 'invoices') {
  ...
  renderContent();
  scheduler.schedule('header', 'theme', 'banner', 'search', 'hero', 'page', 'footer', 'drawer', 'floatingCart', 'modals', 'toast');
  purgeLegacyStorage();
  ...
  void (async () => {
    ...
    scheduler.schedule('header', 'banner', 'page', 'drawer', 'floatingCart', 'modals', 'toast');
