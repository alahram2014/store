...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  try {
    ...
    store.update((draft) => {
      if (Array.isArray(companyCatalog.rows) && companyCatalog.rows.length > 0) {
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'refresh-catalog') {
      removeValue(storageKeys.catalog);
      removeValue(storageKeys.companyRowsCache);
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    ...
    theme: () => { nodes.theme.innerHTML = renderThemeSwitcher(store.getState()); setTheme(store.getState().ui.theme); },
    banner: () => renderBanner(nodes.banner, store.getState()),
    search: () => renderSearchBar(nodes.search, store.getState(), { routeName: store.getState().app.route.name, show: false }),
  ...
  function renderContent() {
    ...
    const hydrating = !state.app.ready || state.runtime.loading.catalog || state.runtime.loading.company || state.runtime.loading.customers || state.runtime.loading.invoices;
    if (hydrating) {
      nodes.page.innerHTML = '<section class="page-section"><div class="empty-state">جارٍ تهيئة البيانات…</div></section>';
