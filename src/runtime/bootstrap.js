...
import { computeCartTotals, getSelectedTier, hasOperationalDashboard } from '../state/selectors.js';
import { createApiClient } from '../services/apiClient.js';
import { loadHomeCatalog, loadCompanyCatalog, aggregateRuntimeProducts, projectRuntimeProducts, projectRuntimeCatalogRows } from '../services/catalogService.js';
...
const companyLoadPromises = new Map();
const companyLoadControllers = new Map();
...
function canAccessDashboard(state) {
  const session = state?.auth?.session || null;
  const isRepDashboard = session?.userType === 'rep' || session?.user_type === 'rep';
  return Boolean(isRepDashboard || hasOperationalDashboard(state));
}
...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  if (companyLoadPromises.has(trimmed)) {
    return companyLoadPromises.get(trimmed);
  }

  for (const [key, controller] of companyLoadControllers.entries()) {
    if (key !== trimmed) {
      try { controller.abort(); } catch { /* noop */ }
      companyLoadControllers.delete(key);
      companyLoadPromises.delete(key);
    }
  }
  ...
  companyLoadControllers.set(trimmed, controller);
  ...
  const promise = (async () => {
    ...
    if (cachedRows.length > 0 && isHydrationSessionActive(hydrationToken) && isRuntimeScopeActive(scope)) {
      store.update((draft) => {
        if (!isRuntimeScopeActive(scope)) return false;
        draft.runtime.companyRowsCache[trimmed] = cachedRows.slice(0, 180);
    ...
    try {
      const companyCatalog = await loadCompanyCatalog(api, trimmed, store.getState().commerce.selectedTier || null, { signal: controller.signal });
      ...
      companyLoadPromises.delete(trimmed);
      companyLoadControllers.delete(trimmed);
      companyHydrationTokens.delete(trimmed);
    }
  })();

  companyLoadPromises.set(trimmed, promise);
  return promise;
}
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'open-invoice-whatsapp') {
      const whatsappUrl = target.getAttribute('data-whatsapp-url');
...
async function performCheckout(store, api, schedule) {
  ...
  try {
    const result = await submitOrder(api, latestState, tier, totals);
    const freshState = store.getState();
    const whatsappUrl = buildWhatsAppInvoice({
      order: result.order,
      items: freshState.commerce.cart,
      session: freshState.auth.session,
    ...
    const invoice = {
      ...
      customer_name: result.customer?.name || freshState.auth.selectedCustomer?.name || freshState.auth.session?.name || '',
    ...
    clearCart();
    persistInvoices([invoice, ...(freshState.commerce.invoices || [])]);
...
export async function bootstrapApp() {
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (nextRoute.name === 'dashboard' && !canAccessDashboard(current)) {
      navigateAuthority(store, 'home');
      return;
    }
    store.patch({
  ...
  // Initial route
  createHydrationSession();
  const initialRoute = parseRoute(location.hash || '#home');
  const effectiveInitialRoute = initialRoute.name === 'dashboard' && !canAccessDashboard(store.getState())
    ? { name: 'home' }
    : initialRoute;
  if (initialRoute.name === 'dashboard' && effectiveInitialRoute.name === 'home') {
    navigateAuthority(store, 'home');
  }
  store.patch({ app: { ...store.getState().app, route: effectiveInitialRoute } });
  ...
  if (currentRoute.name === 'company' && currentRoute.params?.companyId) {
    void ensureCompanyCatalogLoaded(store, api, currentRoute.params.companyId);
  }
  ...
  void (async () => {
    ...
    try {
      const liveCatalog = await loadHomeCatalog(api, bootTier, { includeTop: false, criticalOnly: true, includeProducts: false }).catch(() => null);
