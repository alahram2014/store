...
async function loadCustomersIntoState(store, api, session = null) {
  ...
  if (hasOperationalAccess(rep)) {
    ...
    const rows = await api.get('customers', {
      select: 'id,name,phone,address,location,location_lat,location_lng,username,created_at,sales_rep_id,created_by,created_by_rep_id,customer_type,owner_user_id,owner_user_type',
      order: 'created_at.desc',
      limit: '100',
    }).catch(() => []);
