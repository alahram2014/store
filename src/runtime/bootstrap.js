...
const pendingCompanyCatalogRequests = new Map();
...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  const existing = pendingCompanyCatalogRequests.get(trimmed);
  ...
  const task = (async () => {
    ...
    try {
      const companyCatalog = await loadCompanyCatalog(api, trimmed, store.getState().commerce.selectedTier || null, { pageSize: 24, maxRows: 24 });
      ...
      if (rows.length > 0) {
        store.update((draft) => {
          if (!isRuntimeScopeActive(scope)) return false;
          draft.runtime.companyRowsCache[trimmed] = rows.slice(0, 24);
          const rebuilt = rebuildLoadedCompanyCatalog({ getState: () => draft });
          draft.commerce.catalog.productIndex = rebuilt.productIndex;
          draft.commerce.catalog.products = rebuilt.products;
          draft.commerce.priceBook = rebuilt.priceBook;
      ...
      if (isHydrationSessionActive(hydrationToken) && isRuntimeScopeActive(scope) && fallbackRows.length) {
        store.update((draft) => {
          if (!isRuntimeScopeActive(scope)) return false;
          draft.runtime.companyRowsCache[trimmed] = fallbackRows.slice(0, 24);
          const rebuilt = rebuildLoadedCompanyCatalog({ getState: () => draft });
          draft.commerce.catalog.productIndex = rebuilt.productIndex;
          draft.commerce.catalog.products = rebuilt.products;
          draft.commerce.priceBook = rebuilt.priceBook;
      ...
      pendingCompanyCatalogRequests.delete(trimmed);
    }
  })();

  pendingCompanyCatalogRequests.set(trimmed, task);
  return task;
}
