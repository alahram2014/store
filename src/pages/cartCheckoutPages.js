    ...
    <div class="page-stack">
      ...
      <section class="page-section page-section--summary">
        <div class="summary-box">
          ...
          <button class="btn btn--primary" type="button" data-action="go-checkout">إرسال الطلب</button>
        </div>
      </section>
    </div>
  `;
...
export function renderCheckoutPage(state) {
  const totals = computeCartTotals(state);
  const tier = getSelectedTier(state);
  const sessionType = String(state.auth.session?.userType || state.auth.session?.user_type || state.auth.session?.role || '').trim();
  const customer = getActiveCustomer(state) || (sessionType === 'customer' ? state.auth.session : null);
