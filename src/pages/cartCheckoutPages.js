...
function renderInvoiceLayout({ state, totals, tier, customer, itemsHtml, title, subtitle, actionLabel, actionDisabled, actionAttr, footerNote, invoiceId = null, compact = false, invoiceMeta = '', customerMeta = '', repMeta = '' }) {
  ...
  return `
    <div class="page-stack checkout-page checkout-page--full invoice-page ${compact ? 'invoice-page--compact' : ''}">
      <section class="page-section invoice-hero">
        ...
        ${invoiceMeta || customerMeta || repMeta ? `
          <div class="invoice-hero__meta-grid">
            ${invoiceMeta ? `<div class="invoice-hero__meta">${invoiceMeta}</div>` : ''}
            ${customerMeta ? `<div class="invoice-hero__meta">${customerMeta}</div>` : ''}
            ${repMeta ? `<div class="invoice-hero__meta">${repMeta}</div>` : ''}
          </div>
        ` : ''}
      ...
      <section class="page-section invoice-action">
        <div class="invoice-action__bar">
          ...
          ${actionAttr ? `<button class="btn btn--primary btn--xl" type="button" ${actionAttr} ${actionDisabled ? 'disabled' : ''}>${actionLabel}</button>` : ''}
        </div>
      </section>
    </div>
  `;
...
function renderAddressBlock(invoice) {
  const location = String(invoice.customer_location || invoice.location || '').trim();
  const mapsUrl = String(invoice.customer_google_maps_url || '').trim();
  const lat = Number(invoice.customer_location_lat ?? invoice.location_lat ?? NaN);
  const lng = Number(invoice.customer_location_lng ?? invoice.location_lng ?? NaN);
  const fallbackCoord = Number.isFinite(lat) && Number.isFinite(lng) ? `${lat.toFixed(6)}, ${lng.toFixed(6)}` : '';
  const display = location || fallbackCoord || 'غير محدد';
  const href = mapsUrl || (Number.isFinite(lat) && Number.isFinite(lng) ? `https://maps.google.com/?q=${lat},${lng}` : '');
  return `
    <div class="invoice-detail-card">
      <span>العنوان / الموقع</span>
      <strong>${display}</strong>
      ${href ? `<a class="btn btn--ghost" href="${href}" target="_blank" rel="noopener noreferrer">فتح على Google Maps</a>` : ''}
    </div>
  `;
}

function renderInvoiceMeta(invoice) {
  const customerType = String(invoice.customer_type || '').trim() === 'rep' ? 'عميل مندوب' : 'عميل مباشر';
  return `
    <div class="invoice-detail-card">
      <span>رقم الفاتورة</span>
      <strong>${invoice.order_number || invoice.invoice_number || invoice.id || '—'}</strong>
    </div>
    <div class="invoice-detail-card">
...
export function renderCheckoutPage(state) {
  ...
  return renderInvoiceLayout({
    ...
    invoiceMeta: `<strong>الشريحة:</strong> ${tier.visible_label || tier.tier_name}`,
    customerMeta: `<strong>العميل:</strong> ${customer?.name || 'غير محدد'}`,
    repMeta: customer?.customer_type === 'rep' ? `<strong>نيابة عن:</strong> ${state.auth.session?.system_user?.full_name || state.auth.session?.sales_rep_name || state.auth.session?.name || 'مندوب'}` : '',
  });
}
...
export function renderInvoicePage(state) {
  ...
  const customer = {
    name: invoice.customer_name || 'غير محدد',
  ...
  const repMeta = invoice.customer_type === 'rep' ? `<strong>المندوب:</strong> ${invoice.sales_rep_name || invoice.rep_name || '—'} · <strong>رقم المندوب:</strong> ${invoice.sales_rep_phone || invoice.rep_phone || '—'}` : '';
  const invoiceMeta = renderInvoiceMeta(invoice);
  const customerMeta = `
    <div class="invoice-detail-card">
      <span>العميل</span>
    ...
    <div class="invoice-detail-card">
      <span>تم الإرسال نيابة عن</span>
      <strong>${invoice.sent_on_behalf_of || (invoice.customer_type === 'rep' ? (invoice.sales_rep_name || 'مندوب') : 'العميل نفسه')}</strong>
    </div>
  `;
  const addressBlock = renderAddressBlock(invoice);
  ...
  return renderInvoiceLayout({
    ...
    footerNote: new Intl.DateTimeFormat('ar-EG', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(invoice.created_at || Date.now())),
    invoiceId: invoice.order_number || invoice.invoice_number || invoice.id,
    compact: true,
    invoiceMeta,
    customerMeta: `${customerMeta}${addressBlock}`,
    repMeta,
  });
}
