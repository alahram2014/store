...
function renderCartLine(item) {
  ...
  const unitLabel = item.unitLabel || item.unit || 'قطعة';
  return `
    <article class="invoice-line invoice-line--editable">
      ...
      <div class="invoice-line__content">
        <div class="invoice-line__top">
          <div class="invoice-line__title-block">
            <h3>${item.title || ''}</h3>
            <div class="invoice-line__badges">
              <span class="invoice-line__badge">${unitLabel}</span>
              <span class="invoice-line__badge">${qty} × ${formatMoney(unitPrice)} ج.م</span>
            </div>
          </div>
          <strong class="invoice-line__total">${formatMoney(lineTotal)} ج.م</strong>
        </div>
        <div class="invoice-line__meta">
          <span>الكمية <strong>${qty}</strong></span>
          <span>سعر الوحدة <strong>${formatMoney(unitPrice)} ج.م</strong></span>
        </div>
...
function renderInvoiceLayout({ state, totals, tier, customer, itemsHtml, title, subtitle, actionLabel, actionDisabled, actionAttr, footerNote, invoiceId = null, compact = false }) {
  ...
  const progressLabel = tier.min_order ? `${Math.round(progress)}%` : '100%';
  return `
    <div class="page-stack checkout-page checkout-page--full invoice-page ${compact ? 'invoice-page--compact' : ''}">
      <section class="page-section invoice-hero">
        ...
        <div class="invoice-hero__progress">
          <div class="checkout-progress"><span style="width:${progress}%"></span></div>
          <div class="invoice-hero__progress-meta">
            <span>${formatMoney(totals.grand)} / ${formatMoney(tier.min_order || 0)} ج.م</span>
            <strong>${progressLabel}</strong>
          </div>
        </div>
      </section>
      <section class="page-section invoice-lines">
        <div class="page-section__head page-section__head--tight"><div><h2>تفاصيل الطلب</h2><p>كل صنف يظهر بالكمية والوحدة وسعر الوحدة والإجمالي</p></div></div>
  ...
  return `
    <div class="page-stack">
      <section class="page-section">
        ...
        <div class="summary-box summary-box--compact summary-box--cart">
          ...
          <div><span>التقدم</span><strong>${Math.round(progress)}%</strong></div>
          <div class="checkout-progress"><span style="width:${progress}%"></span></div>
        </div>
        <div class="cart-list">
...
export function renderCheckoutPage(state) {
  ...
  return renderInvoiceLayout({
    ...
    itemsHtml,
    title: 'مراجعة الطلب',
    subtitle: 'ملخص تشغيلي واضح قبل الإرسال إلى واتساب',
...
export function renderInvoicePage(state) {
  ...
  const itemsHtml = items.length ? items.map((item) => {
    ...
    const unitPrice = Number(item.price || 0);
    const lineTotal = qty * unitPrice;
    const unitLabel = item.unit || item.unitLabel || 'قطعة';
    return `
  ...
  return renderInvoiceLayout({
    ...
    subtitle: 'عرض الفاتورة بالتنسيق التشغيلي المختصر',
