...
import { hasCapability, isSalesRepSession } from '../services/authService.js';
...
export function renderCustomersPage(state) {
  ...
  const canManageCustomers = isSalesRepSession(session) || hasCapability(session, ['sales_manager.access', 'sales_manager.assign_customers', 'dashboard.sales_manager', 'dashboard.admin']);
        ...
        ${session ? `
          <div class="account-card">
            ...
            ${Array.isArray(session.capabilities) && session.capabilities.length ? `
              <div class="account-card__row account-card__row--wrap"><span>الصلاحيات</span><strong>${session.capabilities.map((cap) => `<span class="chip">${dom.escape(cap)}</span>`).join(' ')}</strong></div>
            ` : ''}
            <div class="account-card__actions">
              <button class="btn btn--ghost" type="button" data-action="logout">تسجيل الخروج</button>
              ${(Array.isArray(session.capabilities) && session.capabilities.length) ? '<button class="btn btn--ghost" type="button" data-action="go-ops">العمليات</button>' : ''}
