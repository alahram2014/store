    ...
    <article class="invoice-card">
      <div class="invoice-card__top">
        <div>
          ...
          ${invoice.customer_phone ? `<p class="invoice-card__customer">${dom.escape(safeText(invoice.customer_phone))}</p>` : ''}
        </div>
        <strong>${formatMoney(invoice.total_amount || 0)} ج.م</strong>
      </div>
      <div class="invoice-card__meta">
        <span class="chip">${dom.escape(safeText(invoice.customer_type === 'rep' ? 'rep' : invoice.user_type, ''))}</span>
        <span class="chip">${dom.escape(safeText(invoice.status, ''))}</span>
        ${invoice.sales_rep_name ? `<span class="chip">${dom.escape(safeText(invoice.sales_rep_name))}</span>` : ''}
