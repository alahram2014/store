...
function isRenderableCompany(company) {
  return Boolean(company)
    && String(company.company_id ?? '').trim()
...
export function companyCard(company) {
  if (!isRenderableCompany(company)) {
    return '';
  }
...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const sortedUnits = product._sortedUnits || Object.values(product.units || {}).filter((unit) => unit?.unit_code).sort((a, b) => Number(a.display_order ?? 0) - Number(b.display_order ?? 0) || String(a.unit_code).localeCompare(String(b.unit_code), 'ar'));
...
export function offerCard(offer, kind, inCart = false, offerState = null) {
  ...
  const details = offer.description || (isFlash ? 'عرض باكدج محدود ومخصص للتشغيل السريع' : 'عرض تشغيلي جاهز للشراء');
    ...
    <article class="offer-card ${isFlash ? 'offer-card--flash' : 'offer-card--deal'} ${isFlash ? (runtimeStatus === 'active' ? 'is-active' : '') : ''}">
      ...
      <div class="offer-card__body offer-card__body--compact">
        <button class="offer-card__media" type="button" data-action="open-offer" data-offer-type="${kind}" data-id="${offer.id}">
          ${offer.image ? `<img src="${dom.escape(offer.image)}" alt="${dom.escape(title)}" loading="lazy" />` : `<div class="offer-card__image-fallback">${dom.escape(title.slice(0, 1) || 'O')}</div>`}
        </button>
        <div class="offer-card__content">
          ...
          <div class="offer-card__pricebar">
            ...
            <button class="btn btn--primary offer-card__buy" type="button" data-action="${kind === 'deal' ? 'toggle-deal' : 'toggle-flash'}" data-id="${offer.id}" ${isFlash && runtimeStatus !== 'active' ? 'disabled' : offer.can_buy === false ? 'disabled' : ''}>${cta}</button>
          </div>
          <button class="btn btn--ghost offer-card__details" type="button" data-action="open-offer" data-offer-type="${kind}" data-id="${offer.id}">تفاصيل</button>
        </div>
      </div>
    </article>
  `;
