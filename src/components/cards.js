...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const sortedUnits = product._sortedUnits || Object.values(product.units || {}).filter((unitItem) => unitItem?.unit_code).sort((a, b) => Number(a.display_order ?? 0) - Number(b.display_order ?? 0) || String(a.unit_code).localeCompare(String(b.unit_code), 'ar'));
  ...
  const hasDiscount = Number(display.base ?? 0) > Number(display.final ?? 0);
      ...
      <div class="product-card__body">
        <div class="product-card__meta-row">
          <div class="product-card__title-wrap">
            <h3 class="product-card__title">${dom.escape(product.product_name)}</h3>
            <div class="product-card__meta">${dom.escape(product.company_name || '')}</div>
          </div>
          <span class="unit-label unit-label--pill">${dom.escape(labelForUnit(selectedUnit))}</span>
        </div>
        <div class="product-card__price-stack">
          ${hasDiscount ? `<span class="price price--old">${formatMoney(display.base ?? 0)} ج.م</span>` : ''}
          <span class="price ${hasDiscount ? 'price--new' : 'price--main'}">${formatMoney(display.final ?? 0)} ج.م</span>
        </div>
...
export function invoiceCard(invoice) {
  const hasWhatsApp = Boolean(invoice?.whatsapp_url);
  const whatsappLabel = invoice?.whatsapp_delivery_blocked ? 'فتح واتساب يدويًا' : 'إرسال واتساب';
  return `
    <article class="invoice-card">
      <div class="invoice-card__top">
        <div>
          ...
          ${invoice.whatsapp_delivery_blocked ? '<p class="invoice-card__customer">تعذر فتح واتساب تلقائيًا — الرابط متاح هنا</p>' : ''}
        </div>
      ...
      <div class="invoice-card__meta">
        ...
        ${hasWhatsApp ? `<button class="btn btn--ghost invoice-card__action" type="button" data-action="open-invoice-whatsapp" data-invoice-id="${dom.escape(invoice.id)}" data-whatsapp-url="${dom.escape(invoice.whatsapp_url)}">${dom.escape(whatsappLabel)}</button>` : ''}
